...
checkpoint = ModelCheckpoint('../SCRIPTS/TDL/PHYCUV/NEW_MODELS/Not_Augmented/One_Fully_Connected_Layers/Regularization L2/InceptionV3/InceptionV3_1LYR_RegL2_Lr_00001.h5', monitor='val_loss', save_best_only=True, mode='min', verbose=1)
...
# Set up early stopping and model checkpoint callbacks
early_stop = EarlyStopping(monitor='val_loss', patience=5, verbose=1, mode='min', restore_best_weights=True)
checkpoint = ModelCheckpoint('../SCRIPTS/TDL/PHYCUV/NEW_MODELS/Not_Augmented/Two_Fully_Connected_Layers/Regularization L2/InceptionV3/InceptionV3_2LYR_RegL2_Lr_00001.h5', monitor='val_loss', save_best_only=True, mode='min', verbose=1)
...
for fold, (train_index, test_index) in enumerate(kf.split(X)):
    ...
    # Set up early stopping and model checkpoint callbacks
    early_stop = EarlyStopping(monitor='val_loss', patience=5, verbose=1, mode='min', restore_best_weights=True,start_from_epoch=6)
    checkpoint = ModelCheckpoint(f'InceptionV3_REG_L2_2LYR_Lr_00001_fold_{fold}.h5', monitor='val_loss', save_best_only=True, mode='min', verbose=1)
